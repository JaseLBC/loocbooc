...
class Avatar(Base):
    ...
    )  # manual, mobile_scan, lidar, photo_scan
    ...
    # Fit preference profile
    # Stored as JSONB: {"preference": "fitted", "occasions": ["work", "casual"]}
    fit_preference: Mapped[dict] = mapped_column(JSONB, default=dict, nullable=False)

    # Size history — what sizes have worked across different brands
    # {"brand_slug": {"tops": "M", "bottoms": "12", "dresses": "10"}}
    size_history: Mapped[dict] = mapped_column(JSONB, default=dict, nullable=False)

    # Style profile (colours, silhouettes, occasions)
...
class AvatarMeasurement(Base):
    ...
    arm_length_cm: Mapped[float | None] = mapped_column(Float)
    ...
    # Body type derived from measurements
    body_type: Mapped[str | None] = mapped_column(String(50))

    # Measurement source and confidence
    measurement_source: Mapped[str] = mapped_column(
        String(50), nullable=False, default="manual"
    )  # manual, photo_scan, tape_measure, ml_inferred
    confidence_score: Mapped[float | None] = mapped_column(Float)  # 0-1

    # Extended measurements from LiDAR/scan or photo estimation
